 ...
 * 		
 * 		$image = image::process( 'resize', array( 'source_image' => $source_image, 'width' => 200, 'height' => 200, 'create_thumb' => true ) );
 * 		
 * 		if ( $image -> is_error() )
 * 			throw new Exception( $image -> get_error() );
 * 		
 * 		// Абсолютный путь к файлу
 * 		$file_path = $image -> get_file_path();
 * 		
 * 		// Относительный путь к файлу
 * 		$file_link = $image -> get_file_link();
 */
class image
{
	...
	(
		'ru' => array
		(
			'imglib_source_image_required' => 'Не задано исходное изображение',
			'imglib_gd_required' => 'Не установлена библиотека GD',
			'imglib_gd_required_for_props' => 'Не установлена библиотека GD',
			'imglib_unsupported_imagecreate' => 'Библиотека GD не поддерживает этот тип изображений',
			'imglib_gif_not_supported' => 'Не поддерживаются функции для работы с GIF-изображениями',
			'imglib_jpg_not_supported' => 'Не поддерживаются функции для работы с JPG-изображениями',
			'imglib_png_not_supported' => 'Не поддерживаются функции для работы с PNG-изображениями',
			'imglib_rotate_unsupported' => 'Не поддерживаются функции для поворота изображений',
			'imglib_libpath_invalid' => 'Неверно указан путь к библиотеке',
			'imglib_image_process_failed' => 'Процесс обработки изображения прерван',
			'imglib_rotation_angle_required' => 'Не задан угол поворота изображения',
			'imglib_invalid_path' => 'Не задано исходное изображение',
			'imglib_missing_font' => 'Не задан шрифт для водяного знака',
			'imglib_save_failed' => 'Невозможно сохранить изображение',
		),
		'en' => array
		(
	 ...
	 * @return	object
	 */
	static function process( $action = 'resize', $props = array() )
	{
		$obj = new Image( $props );
		
		if ( in_array( $action, array( 'resize', 'crop', 'rotate', 'watermark' ) ) )
			$obj -> $action();
	...
	function image_process_gd($action = 'resize')
	{
		...
		if ($action == 'crop')
		{
			if ( $this->width == $this->old_width )
			{
				...
				$orig_width = $this->old_width*$this->orig_height/$this->old_height;
				
				$this->x_axis = ($this->orig_width-$orig_width)/2;
				$this->y_axis = 0;
			...
			else
			{
				...
				$orig_height = $this->old_height*$this->orig_width/$this->old_width;
				
				$this->x_axis = 0;
				$this->y_axis = ($this->orig_height-$orig_height)/2;
		...
		if ( $this -> save_alpha )
		{
			...
			{
				imagecolortransparent($dst_img, imagecolorat($dst_img, 0, 0));
			}
	...
	function image_create_gd($path = '', $image_type = '')
	{
		...
		switch ($image_type)
		{
			case	 1 :
						...
						{
							$this->set_error('imglib_gif_not_supported');
							return FALSE;
						}
			...
			case 2 :
						...
						{
							$this->set_error('imglib_jpg_not_supported');
							return FALSE;
						}
			...
			case 3 :
						...
						{
							$this->set_error('imglib_png_not_supported');
							return FALSE;
						}
